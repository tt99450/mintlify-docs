---
title: "What's new in BigBlueButton (Spring 2023)"
description: "This document covers what's new in the Spring 2023 release. What's new in BigBlueButton 2.6: Engagement New whiteboard Downloading presentation with..."
---

#### This document covers what's new in the Spring 2023 release.

#### What's new in BigBlueButton 2.6

- Engagement
  - New whiteboard
  - Downloading presentation with whiteboard annotations included
  - Exporting shared notes to the whiteboard
  - Exporting breakout rooms’ shared notes to the main room
  - Exporting breakout rooms’ whiteboard annotations to the main room
  - Pin shared notes to the whiteboard
  - Easier setup of polling
  - Easier moving of users between breakout rooms
- Management
  - Propagate layout to all participants
  - Improved layout manager selection
  - Upload a presentation directly from NextCloud
- Personalization
  - Enable/Disable dark mode
  - Pinning webcams
  - Upload custom webcam background
  - Set webcam image brightness
  - Modify screen layout
- [Automatic closed captions for speaker](/help/user-guide/view-closed-captions)

# 

# Engagement

## New whiteboard

The whiteboard has been replaced with new, modern whiteboard that supports all the capabilities you would expect, including

- moving/resizing
- erase
- duplicate
- editing

Here's an overview video of the new whiteboard capabilities.

For a list of shortcuts click on the three dots in the top right corner select Keyboard shortcuts, then select the Whiteboard option on the left-hand side.

|  |  |
| --- | --- |
| mceclip8.png | mceclip9.png |

There are two toolbars on the whiteboard.  A Styles toolbar and a Tools toolbar.

Styles toolbar

![mceclip10.png](/images/11974444132109)

|  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | mceclip12.png   |  |  | | --- | --- | | Color | Select the color for your annotations to display in | | Fill | If selected objects will be filled | | Dash | Select the dash pattern of your object | | Size | Select your font size (S)mall, (M)edium or (L)arge | | Keep Open | Keeps the Styles toolbar open | |
| 2 | Undo |
| 3 | Redo |
| 4 | Options:   |  |  | | --- | --- | | Fullscreen Presentation | Make the presentation space fullscreen | | Snapshot of current slide | Creates and downloads an image of the current slide with annotations | |

Tools toolbar

|  |  |
| --- | --- |
| mceclip13.png | mceclip14.png mceclip15.png |

|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | |  |  | | --- | --- | | mceclip16.png | Duplicate | | mceclip17.png | Rotate | | mceclip18.png | Unlock | | mceclip19.png | Unlock Aspect Ratio | | mceclip20.png | Group | | mceclip21.png | Move to Back | | mceclip22.png | Move Backward | | mceclip23.png | Move Forward | | mceclip24.png | Move to Front | | mceclip25.png | Reset Angle | | mceclip26.png | Align Left | | mceclip27.png | Align Horizontal Center | | mceclip28.png | Align Right | | mceclip29.png | Stretch Horizontal | | mceclip30.png | Distribute Horizontal | | mceclip31.png | Align Top | | mceclip32.png | Align Vertical Center | | mceclip33.png | Align Bottom | | mceclip34.png | Stretch Vertical | | mceclip35.png | Distribute Vertical | |
| 2 | Selection - select annotations |
| 3 | Pen - freehand line drawing |
| 4 | Eraser - swipe across objects to erase them |
| 5 | Select a shape to use rectangle, ellipse, triangle or line - drop and drag to size |
| 6 | Arrow - draw an arrow |
| 7 | Text - type text |
| 8 | Sticky Notes - place a sticky note to type on |
| 9 | Delete the selected annotation |

## 

## Downloading presentation with whiteboard annotations included

Educators can now export their slide decks with the annotations included during a lesson. Selecting “Send to chat” in the presentation upload modal sends a link to the public chat from which meeting participants can use to download the file.

![mceclip5.png](/images/11973969189645)

## Exporting shared notes to the whiteboard

Instructors can move the shared notes to the whiteboard presentation area for further collaborative work among students.  To do this select the three dots in the top right corner of the Shared Notes window and select Convert notes to presentation.  The Shared Notes will then be converted and displayed in the presentation area.

![mceclip37.png](/images/11975228194701)

The Shared Notes will then be available as a presentation.

![mceclip39.png](/images/11975272283661)

Which can then be marked up on the whiteboard.

![mceclip38.png](/images/11975248414093)

## 

## Exporting breakout rooms’ shared notes to the main room

To capture a breakout room's shared notes, select the option “Capture shared notes when breakout rooms end” while setting up the breakout rooms.

When the breakout rooms end the shared notes will be converted to a pdf and be available as a presentation from the action menu.

![mceclip40.png](/images/11975346012045)

## Exporting breakout rooms’ whiteboard annotations to the main room

To capture the breakout rooms' shared notes, select the option “Capture whiteboard when breakout rooms end” while setting up the breakout rooms.

When the breakout rooms end the whiteboard will be converted to a pdf and be available as a presentation from the action menu.

![mceclip41.png](/images/11975368245517)

## 

## Pin shared notes to the whiteboard

Instructors can pin the shared notes to the whiteboard presentation area for further collaborative work among students.  To do this select the three dots in the top right corner of the Shared Notes window and select Pin notes onto whiteboard.

![mceclip2.png](/images/12006831420813)

To unpin the shared notes, press the X button on the top right-hand side of the whiteboard area.

![2.6_-_Pin_Shared_Notes_to_Whiteboard.gif](/images/12006876226957)

## Easier setup of polling

Polling has been updated to allow for polls to be prepared in advance using the custom input option.

![2.6_-_Custom_Polling.gif](/images/11996235264525)

## Easier moving of users between breakout rooms

There is a new option for the breakout room controls called Manage users.

![mceclip6.png](/images/11974014536205)

This dialog lets you drag and drop users between different breakout rooms.

![2.6_-_BO_-_Move_Users.gif](/images/11974091374093)

When you click the Apply button, BigBlueButton will send prompts to each user to move them to the target breakout room.![mceclip7.png](/images/11974282585869)

## 

# Management

## Propagate layout to all participants

Moderators can propagate their layout to all participants.  To do this select the actions menu by pressing the (+) button and then select Propagate layout

![mceclip13.png](/images/12034170473101)

## Improved layout manager selection

To change the layout of the screen, select the actions menu by pressing the (+) button and then select Layout Settings Modal.

![mceclip2.png](/images/11973769049229)

You will be presented with 4 options, Custom, Smart layout, Focus on presentation, and Focus on video.

The custom layout will allow you to drag the webcams to the top, bottom, and either side of the screen.

The smart layout will select will display the webcams at the top of the screen with the presentation area being directly below them.

Focus on presentation will use the entire main screen for presentations and display webcams at the bottom of the Public Chat window.

Focus on video will use the entire main screen for webcams and display the presentation at the bottom of the Public Chat window.

![mceclip4.png](/images/11973909212941)

## Upload a presentation directly from NextCloud

Two newly created parameters: presentationUploadExternalDescription and presentationUploadExternalUrl have been introduced.

When both of these parameters have been populated a message in the presentation upload modal will appear.

![mceclip14.png](/images/12127170234125)

# 

## 

## Improved echo test dialog

The echo test dialog has been updated, adding an audio stream volume indicator.

![mceclip4.png](/images/11962375461133)

## Improved notification of recording start

The recording indicator has been made bigger.

![mceclip5.png](/images/11962725948941)

# Personalization

## Enable/Disable dark mode

To enable/disable dark mode click on the 3 vertical dots [1] in the upper right-hand corner and select Settings.

![mceclip0.png](/images/11962020111501)

Under the Application panel [1], you will find the Dark mode setting [2].

Use the slider next to Dark mode to toggle between on/off.

Click the Save button [3] to apply the setting.

![mceclip1.png](/images/11962192971021)

![mceclip1.png](/images/12005313979661)

## Pinning webcams

You can pin one or more webcam(s), so they always stay visible. This is useful if one of the webcams is showing sign language, for example, and you always want it to be visible.

![2.6_-_Webcam_Pin.gif](/images/11973711051917)

## Upload custom webcam background

You can upload a custom background image from the Webcam settings page.  Click on the camera icon, then the plus button.  

![mceclip0.png](/images/11973604534797)

You will then be able to select a background image to upload from your computer.  The selected image will show up in the first position on the left in the list of available backgrounds to use. 

![2.6_-_Custom_Background.gif](/images/11973411160205)

To delete a custom background, click the X in the upper right-hand corner of the image and it will be removed from your background selections.

![mceclip1.png](/images/11973643189261)

## Set webcam image brightness

You can adjust the brightness of your image or the entire image as a whole using the Brightness slider bar at the bottom of the Webcam settings window.

If the Whole image radio button is selected it will adjust the brightness of the whole image, if it isn’t selected only your image will be adjusted.

![2.6_-_Webcam_brightness.gif](/images/11973702375053)

## Modify screen layout

You can customize the webcam location by clicking and dragging to a new position (top, bottom, left, right or to the bottom of the shared notes).

![mceclip11.png](/images/12033451412237)

# Accessibility

## Live Automatic Closed Captions

**Note:** This feature uses built-in Google APIs to enable live speech-to-text for a user.  This feature is disabled by default.  When BigBlueButton 2.6 release, to have this feature enabled for your account, contact Blindside Networks support. 

BigBlueButton now supports per-user captioning.  That is, when a user joins the audio, they can chooses which language they are speaking and, using Google's speech-to-text ability, BigBlueButton will make live transcription of their speech available to others.  These captions will also appear in the recordings.

To have your voice transcribed, select the desired language from the drop-down selector from the join audio dialog.

![mceclip3.png](/images/12025525358605)

When a user selects a language for automatic transcription, a CC icon will display next to their name in the “who is talking” indicator.

![mceclip4.png](/images/12026037739533)

When one or more users have enabled transcription, a CC button will appear.  To view the transcriptions for those with ‘CC’ in their “who is talking” icon, press the CC button.  You don’t need to have selected transcription of your audio to view the transcriptions for others.

The placement of the ‘CC’ button depends on your device.  For mobile devices, the ‘CC’ button is in the three dots at the top right corner.

![mceclip7.png](/images/12027795950861)

For desktop, the ‘CC’ button is in the bottom left.

![mceclip5.png](/images/12026422463373)

Desktop users can also select the language that the captioning will display in by pressing the up arrow on the CC button.

![mceclip6.png](/images/12026719839885)

Note: The automatic transcription is only available for browsers that support SpeechRecognition (Google Chrome, MS Edge, and Safari). If you join with non-supported browsers, you will see a warning at the bottom of the join audio dialog.

![mceclip9.png](/images/12028083485325)

## 